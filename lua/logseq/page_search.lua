...
-- Helper to decode Logseq's weird filename quirks
...
-- Advanced Matcher: Calculates a relevance score
...
local function get_all_pages()
  ...
  local function scan_dir(dir)
    ...
    for _, file in ipairs(files) do
      ...
      table.insert(items, decode_filename(basename))
    end
  end
...
-- Neovim's native completion function engine
...
-- The setup function called by your init.lua
function M.setup_buf(bufnr)
  ...
  -- THE FORCE-REFRESH HACK: Live fuzzy filtering as you type
  ...
  vim.api.nvim_create_autocmd("TextChangedI", {
    ...
    callback = function()
      ...
      -- Check if we are typing inside an unclosed [[ block
      if text_before:match("%[%[[^%]]*$") then
        -- vim.schedule prevents Vim from throwing textlock errors while typing fast
        vim.schedule(function()
          -- If the menu is already open, <C-e> kills it so <C-x><C-u> can cleanly restart it.
