...
function M.winbar()
  ...
  if wb.backlinks ~= false then table.insert(nav_parts, "%@v:lua.logseq_panel_backlinks@b🖇️%X")   end
  if wb.queries   ~= false then table.insert(nav_parts, "%@v:lua.logseq_panel_queries@q❔%X")       end
  if wb.ns_tree   ~= false then table.insert(nav_parts, "%@v:lua.logseq_panel_ns_tree@n🌲%X")       end
